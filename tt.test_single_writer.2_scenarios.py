...
def test_writer(wid, model_file, folder,text_corpus,data_dict):
    ...
    # print("imgs.size: '{}' ".format(imgs.size()))  #imgs.size: 'torch.Size([1, 50, 64,
    # random.shuffle(imgs)


    # with open(text_corpus, 'r') as _f:
    #     texts = _f.read().split()
    texts = [i[1] for i in data_dict[wid]]
    ...
    with torch.no_grad():
        ...
        for label in labels:
            ...
            # cv2.imwrite(img_folder + '_real/' + wid + '-' + str(num) + '.' + label + '.png', img)
...
if __name__ == '__main__':
    for i in range(2):
        if i == 0:
            folder = folder_pre + '/' + model_epoch + '_3/' + 'res_1.in_vocab_tr_writer'
            target_file = 'Groundtruth/gan.iam.tr_va.gt.filter27'
            text_corpus = 'corpora_english/in_vocab.subset.tro.37'
        elif i ==1:
            folder = folder_pre + '/' +model_epoch + '_3/' +'res_2.in_vocab_te_writer'
        ...
        # test_writer(wids[0], 'save_weights/contran-' + model_epoch + '.model', folder, text_corpus, data_dict)
        wids = tqdm(wids)
        for wid in wids:
