...
parser.add_argument('--in_oov', type=int, default=0,
                    help='in_oov to use')
...
if __name__ == '__main__':
    ...
    if in_oov == 1:
        ...
        x = ''
    father_path = os.path.abspath(os.path.dirname(args.path[0]) + os.path.sep + ".")
    wids = os.listdir(args.path[0])
    wids.sort()
    wids = tqdm(wids)
    for wid in wids:
        path_wid = []
        path_wid.append(args.path[0] + '/' + str(wid))
        path_wid.append(args.path[1] + '/' + str(wid))

        fid_value = calculate_fid_given_paths(path_wid,
                                          args.batch_size,
                                          args.gpu != '',
                                          args.dims)
