        ...
        if epoch % EVAL_EPOCH == 0:
            ...
            # generator eval() to avoid BN crash on tiny batches
                ...
                for batch in test_loader:
                    ...
                    # 1) Generate using writertest-equivalent path (readable)
                    with torch.no_grad():
                    ...
                    if xg is None:                # nothing passed the rec filter
                        skipped_small += 1
                        continue

                    # 2) TroCR pseudo labels (try both polarities; pick the better one)
                    ...
                    # 4) Build labels for recognizer (optionally restrict to your vocab)
                    texts_raw   = [texts[i] for i in range(len(texts)) if mask[i]]
                    #log(f"[Teacher Step] TroCR RAW predictions: {texts}")
                    #log(f"[Teacher Step] TroCR RAW (kept {len(texts_raw)}): {texts_raw[:11]}")
                    texts_clean = [clean_for_vocab(t) for t in texts_raw]   # keep if your HTR is letters-only
                    labels      = texts_to_labels(texts_clean)

                    # 5) Recognizer forward on survivors
                    xg_sel = xg[mask]                  # recognizer expects [-1,1]
                    rec_logits = recognition_logits(model, xg_sel, labels["ids"], labels["img_width"])
                    loss_rec_pseudo = recognition_loss(rec_logits, labels)

                    # 6) Weight by confidence & optimize recognizer (R-only)
                    ...
                    # 7) Save a few samples (visually aligned with writertest)
                    #tgt_sel  = [w for i, w in enumerate(words_k) if mask[i]]
                    # if steps % 2 == 0 and xg_sel.size(0) > 0:
                    #     save_teacher_samples(
                    #         xg_sel, texts_clean, tgt_sel,
                    #         out_dir=os.path.join(logdir, "teacher_samples"),
                    #         epoch=epoch, step=steps, max_n=11, invert=True
                    #     )
