...
lr_dis = 1 * 1e-4
lr_gen = 1 * 1e-4
#lr_rec = 1 * 1e-5  # Use a lower LR for the unfrozen TrOCR decoder
lr_cla = 1 * 1e-5
...
def train(train_loader, model, dis_opt, gen_opt, rec_opt, cla_opt, epoch):
    ...
    rec_frozen = (rec_opt is None)  # Check if recognizer optimizer is None (meaning frozen)
    ...
    for train_data_list in train_loader:
        # --- Recognizer Update (Only if NOT frozen) ---
        if not rec_frozen:
            rec_opt.zero_grad(set_to_none=True)
            # This call performs forward+loss+backward for the recognizer
            l_rec_tr = model(train_data_list, epoch, 'rec_update', cer_tr)
            rec_opt.step()
        else:
            # Recognizer is frozen, skip update and set loss to 0
            #l_rec_tr = torch.tensor(0.0, device=device)
            l_rec_tr = None

    #    with torch.no_grad():
    #        l_rec_tr = torch.tensor(0.0, device=device)
        # --- Classifier Update ---
        cla_opt.zero_grad(set_to_none=True)
    ...
    # Calculate CER, guarding against empty batches
    res_cer_tr = cer_tr.fin() if getattr(cer_tr, "len", 0) > 0 else 0.0
    res_cer_te = cer_te.fin() if getattr(cer_te, "len", 0) > 0 else 0.0
    res_cer_te2 = cer_te2.fin() if getattr(cer_te2, "len", 0) > 0 else 0.0
    ...
    #       np.mean(loss_rec), np.mean(loss_l1),
    #       res_cer_tr, res_cer_te, res_cer_te2, time.time() - time_s)))
    
    # return res_cer_te + res_cer_te2

    if rec_frozen or len(loss_rec_tr) == 0:
        log(('epo%d <tr>-<gen>: l_dis=%.2f-%.2f, l_cla=%.2f-%.2f, l_rec=NA-%.2f, l1=%.2f, '
             'cer=%.2f-%.2f-%.2f, time=%.1f') %
            (epoch, np.mean(loss_dis_tr), np.mean(loss_dis),
             np.mean(loss_cla_tr), np.mean(loss_cla),
             np.mean(loss_rec), np.mean(loss_l1),
             res_cer_tr, res_cer_te, res_cer_te2, time.time() - time_s))
    else:
        log(('epo%d <tr>-<gen>: l_dis=%.2f-%.2f, l_cla=%.2f-%.2f, l_rec=%.2f-%.2f, l1=%.2f, '
             'cer=%.2f-%.2f-%.2f, time=%.1f') %
            (epoch, np.mean(loss_dis_tr), np.mean(loss_dis),
             np.mean(loss_cla_tr), np.mean(loss_cla),
...
def main(train_loader, test_loader, num_writers):
    ...
    for epoch in range(CurriculumModelID, 50001):
        ...
        if epoch % EVAL_EPOCH == 0:
            ...
            # Update recognizer scheduler if it exists
...
# ... [The rest of your code for optuna_objective, train_with_custom_lr, rm_old_model] remains unchanged] ...
# [Include your optuna_objective, train_with_custom_lr, and rm_old_model functions here]
