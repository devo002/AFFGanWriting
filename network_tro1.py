...
class ConTranModel(nn.Module):
    ...
    def forward(self, train_data_list, epoch, mode, cer_func=None):
        ...
        if mode == 'rec_update':
            ...
            self.iter_num += 1
            '''dis loss'''
            f_xss = self.gen.enc_image(tr_img) # b,512,8,27
            f_xs = f_xss[-1]

            f_xt, f_embed = self.gen.enc_text(label_xt, f_xs.shape) # b,4096  b,512,8,27
            f_mix = self.gen.mix(f_xss, f_embed)

            xg = self.gen.decode(f_mix, f_xss, f_embed, f_xt)  # translation b,1,64,128
            ...
            xg_swap = self.gen.decode(f_mix_swap, f_xss, f_embed_swap,f_xt_swap)  # translation b,1,64,128
