...
from recognizer.models.encoder_vgg import ResNet50Encoder, EncoderConvNeXt
...
def write_image(xg, pred_label, gt_img, gt_label, tr_imgs, xg_swap, pred_label_swap, gt_label_swap, title, num_tr=2):
    folder = '/home/woody/iwi5/iwi5333h/img11'
...
class GenModel_FC(nn.Module):
    def __init__(self, text_max_len):
        ...
        self.enc_text = TextEncoder_FC(text_max_len=52).to(gpu)
...
convnext_weights = "/home/woody/iwi5/iwi5333h/model/convnext_tiny-983f1562.pth"
...
class ConvNeXtTiny(nn.Module):
    def __init__(self, weight_path=None, in_channels=50):
        ...
        # Modify first conv layer for in_channels
        ...
        # Use only Conv2D-based blocks for feature extraction
        return_nodes = {
            'features.1': 'feat1',  # 96 channels
            'features.2': 'feat2',  # 192 channels
            'features.3': 'feat3',  # 384 channels
            'features.4': 'feat4',  # 768 channels
        }
        self.extractor = create_feature_extractor(self.model, return_nodes=return_nodes)

        # Reduce to 512 channels
        self.reduce_layers = nn.ModuleList([
            nn.Conv2d(96, 512, kernel_size=1),    # feat1
            nn.Conv2d(192, 512, kernel_size=1),   # feat2
            nn.Conv2d(384, 512, kernel_size=1),   # feat3
            nn.Conv2d(768, 512, kernel_size=1),   # feat4
        ])

    def encode_with_intermediate(self, x):
        features = self.extractor(x)
        results = []
        raw_feats = []  # store raw features

        for i, key in enumerate(['feat1', 'feat2', 'feat3', 'feat4']):
            raw_feat = features[key]             # e.g. [B, 768, H, W]
            raw_feats.append(raw_feat)

            reduced = self.reduce_layers[i](raw_feat)  # [B, 512, H, W]
            results.append(reduced)

    # results[-1] = reduced version [B, 512, 8, 27]
    # raw_feats[-1] = original 768-dim version [B, 768, 4, 13] or similar

        results[-1] = F.interpolate(results[-1], size=(8, 27), mode='bilinear', align_corners=False)
        return results

    def forward(self, x):
        out = self.encode_with_intermediate(x)
...
vit_weights_path = "/home/woody/iwi5/iwi5333h/model/vit_b_16-c867db91.pth"   
class VisionTransformerEncoder(nn.Module):
    def __init__(self, weight_path=None, in_channels=50, patch_size=16, image_size=(64, 864)):
        super(VisionTransformerEncoder, self).__init__()
        self.output_dim = 512  # For compatibility

        # Load a pretrained ViT model and discard classifier
        self.model = vit_b_16(weights=None)
        if weight_path:
            state_dict = torch.load(weight_path, map_location="cpu")
            self.model.load_state_dict(state_dict)

        # Replace the first conv layer to support in_channels (custom input channels)
        ...
        with torch.no_grad():
            ...
            if in_channels > 3:
                extra = old_proj.weight[:, :1].repeat(1, in_channels - 3, 1, 1)
                self.model.conv_proj.weight[:, 3:] = extra

        # Remove classification head
        self.model.heads = nn.Identity()

        # Project transformer output to 512 channels
        self.output_projector = nn.Linear(self.model.hidden_dim, self.output_dim)

    def forward(self, x):
        B, C, H, W = x.size()
        x = self.model.conv_proj(x)  # -> [B, hidden_dim, H', W']
        h, w = x.shape[2:]

        x = x.flatten(2).transpose(1, 2)  # [B, num_patches, hidden_dim]
        x = self.model.encoder(x)        # [B, num_patches, hidden_dim]

        x = self.output_projector(x)     # [B, num_patches, 512]
        x = x.transpose(1, 2).reshape(B, self.output_dim, h, w)

        # Resize to fixed shape for GenModel compatibility
        x = F.interpolate(x, size=(8, 27), mode='bilinear', align_corners=False)
        return [x] * 6  # mimic VGG/EfficientNet/ResNet with 6-level features
