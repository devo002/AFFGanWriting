...
def find_form_path(root, name):
    import os
    # try name, name+'u', name+'x' with common extensions
    bases = [name, f"{name}u", f"{name}x"]
    exts  = [".png", ".tif", ".tiff", ".jpg", ".jpeg"]
    # IAM form dirs
    dirs  = ["forms", "formsA-D", "formsE-H", "formsI-Z"]
    # some distributions have a subfolder equal to the first 3 chars (e.g., c06)
    subdirs = ["", name[:3]]

    candidates = []
    for d in dirs:
        for sd in subdirs:
            base_dir = os.path.join(root, d, sd) if sd else os.path.join(root, d)
            for b in bases:
                for ext in exts:
                    p = os.path.join(base_dir, b + ext)
                    candidates.append(p)
                    if os.path.exists(p):
                        return p
    raise FileNotFoundError("Form image not found for {name}. Tried:\n" + "\n".join(candidates))
...
class AutoTrainer(BaseTrainer):
    ...
    # Always use a single iterator over the single train loader
        loader = self.data_loader
        if self._data_loader_iter is None:
            self._data_loader_iter = iter(loader)

    # Compute lesson first (you referenced `lesson` before assigning earlier)
        lesson = None
        if getattr(self, "curriculum", None):
            lesson = self.curriculum.getLesson(iteration)

    # Get a batch: curriculum may override fetching from the loader
        if lesson and all(l[:3] == "gen" for l in lesson) and getattr(self, "text_data", None) is not None:
            instance = self.text_data.getInstance()
        else:
            try:
                instance = next(self._data_loader_iter)
        ...
        # weights use keys without the "Loss" suffix (e.g., 'autoLoss' -> 'auto')
...
#         """
#         if self._data_loader_iter is None:
#         # use the name your code uses for the train loader:
#         # e.g., self.train_loader  OR  self.data_loader['train']  OR  self.data_loader
#             self._data_loader_iter = iter(self.train_loader)

#         try:
#             instance = next(self._data_loader_iter)
#         except StopIteration:
#         # reset when the loader is exhausted
#             self._data_loader_iter = iter(self.train_loader)
#             instance = next(self._data_loader_iter)

    
#         # --- choose the correct loader/iterator ---
#         loader = self.data_loader
#         if not hasattr(self, "data_loader_iter") or self.data_loader_iter is None:
#             self.data_loader_iter = iter(loader)

# # curriculum may override fetching from the loader
